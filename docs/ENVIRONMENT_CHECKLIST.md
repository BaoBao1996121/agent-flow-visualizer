...
|---|---:|---|---|
| Python | 3.11–3.13 | Local `Python 3.13.1`; `387 passed, 1 skipped` in 19.71s | Run 29639913312: 3.11, 3.12, and 3.13 PASS. |
...
| Staged validation runner | Python 3.12 and Node 22 for shadow S1 | Exact shadow command passed once in 5.50s on the warm Windows environment; focused CI contract `2 passed`; no canonical S0 runner or machine-readable stage manifest exists yet | `Exploration fast gate` and aggregate are working-branch configuration only; hosted timings pending. |
| GitHub validation control plane | Strict status checks, admin enforcement, GitHub Actions app contexts | Live API baseline: personal-account repository, no ruleset/merge queue, nine required contexts | The nine-context protected-main baseline is unchanged. Aggregate migration must follow shadow evidence and API readback. |
