...
- latest-code local manual Chromium verification of LangGraph JSON import; NDJSON, AG-UI, Demo, sequence-20 seek, Fork, and Compare are earlier same-day manual evidence, while hosted browser automation remains pending;
- real LangGraph `1.1.0` and `1.2.9` runtime probes across `tasks`, `messages`, `updates`, `values`, `checkpoints`, and `custom`.

## Next milestones

1. Execute visual Phase -1: remove decorative states without evidence, freeze the visual truth grammar, and record the Canvas baseline described in [VISUAL_SYSTEM.md](VISUAL_SYSTEM.md).
